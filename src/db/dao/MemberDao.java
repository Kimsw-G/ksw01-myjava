...
public class MemberDao extends DBCon {
    ...
    public void updateMember(MemberEntity memberEntity){
        ...
        try {
            conn = conn();
            final String SQL = "update member set mname=?, mpass=?, mphone=? where mno=? ";
            pstmt = conn.prepareStatement(SQL);
            pstmt.setString(1, memberEntity.getMname());
            pstmt.setString(2, memberEntity.getMpass());
            pstmt.setString(3, memberEntity.getMphone());
            pstmt.setInt(4, memberEntity.getMno());
            pstmt.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }finally{
